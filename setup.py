...
THIRD_PARTY = os.path.join(BASEDIR, "third_party")
BUILD_DIR = os.path.join(BASEDIR, "build")  # CMake build root (already populated)
...
# Relative paths (setuptools requires relative sources; -I flags below are
# absolute so the compiler finds headers regardless of ninja's cwd).
EXT_SOURCES = [
...
EXT_INCLUDE_DIRS = [
    ...
    os.path.join(BASEDIR, "src", "ps", "proto"),
    os.path.join(BASEDIR, "src", "ps", "grpc"),
]
...
def _prepare_vendored_native_libs_for_wheel() -> None:
    ...
    """
    native_dir = os.path.join(BASEDIR, "src", "python", "pytorch", "recstore", "native")
...
class BuildPyCommand(build_py):
    ...
    def _render_version_info(self) -> None:
        ...
        src_pkg = os.path.join(BASEDIR, "src", "python", "pytorch", "recstore")
...
# Custom build_ext that ensures vendored .so files are copied for wheel builds
# and ensures the extension .so lands in recstore/ (next to __init__.py).
class RecStoreBuildExt(BuildExtension):
    """BuildExtension subclass that copies vendored .so files for wheels."""
...
if __name__ == "__main__":
    ...
    setup(
        ...
        packages=find_packages(where="src/python/pytorch"),
        package_dir={"": "src/python/pytorch"},
        package_data={
            "recstore": [
                "native/*.so",
                "native/*.a",
