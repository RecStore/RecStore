...
# RecStore Integration
try:
    from recstore_embedding_layer import RecStoreEmbeddingLayer
    print("RecStore embedding layer imported successfully")
except ImportError as e:
    print(f"Warning: Could not import RecStore embedding layer: {e}")
    RecStoreEmbeddingLayer = None
...
class DLRM():
    def __init__(self,
        ...
        self.use_recstore = use_recstore
    ...
    def _create_model(self):
        ...
        with tf.compat.v1.variable_scope('input_layer', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            with tf.compat.v1.variable_scope('sparse_input_layer', reuse=tf.compat.v1.AUTO_REUSE):
                column_tensors = {}
                
                if self.use_recstore and RecStoreEmbeddingLayer is not None:
                    print("Using RecStore backend for embedding storage with training support")
                    
                    # Create RecStore embedding layer
                    self.recstore_embedding_layer = RecStoreEmbeddingLayer(
                        embedding_dim=16,
                        name_prefix="dlrm_recstore_emb"
                    )
                    ...
                    sparse_inputs = tf.concat(sparse_embeddings, axis=1)
                elif self._adaptive_emb and not self.tf:
        ...
        mlp_bot_scope = tf.compat.v1.variable_scope(
            'mlp_bot_layer',
        ...
        with mlp_bot_scope.keep_weights(dtype=tf.float32) if self.bf16 \
            ...
            for layer_id, num_hidden_units in enumerate(self._mlp_bot):
                with tf.compat.v1.variable_scope(
                        ...
                        reuse=tf.compat.v1.AUTO_REUSE) as mlp_bot_hidden_layer_scope:
                    dense_layer = tf.keras.layers.Dense(
                        units=num_hidden_units,
                        activation=tf.nn.relu,
                        name=str(mlp_bot_hidden_layer_scope.name))
                    dense_inputs = dense_layer(dense_inputs)
                    bn_layer = tf.keras.layers.BatchNormalization(
                        trainable=True)
                    dense_inputs = bn_layer(dense_inputs)
        ...
        if self.interaction_op == 'dot':
            ...
            # Ensure sparse_inputs is float32 for concatenation
            if sparse_inputs.dtype != tf.float32:
                sparse_inputs = tf.cast(sparse_inputs, tf.float32)
            mlp_input = tf.concat([dense_inputs, sparse_inputs], 1)

        # top MLP before output
        if self.bf16:
            mlp_input = tf.cast(mlp_input, dtype=tf.bfloat16)
        mlp_top_scope = tf.compat.v1.variable_scope(
            'mlp_top_layer',
            partitioner=self._dense_layer_partitioner,
            reuse=tf.compat.v1.AUTO_REUSE)
            ...
            for layer_id, num_hidden_units in enumerate(self._mlp_top):
                with tf.compat.v1.variable_scope(
                    ...
                    dense_layer = tf.keras.layers.Dense(
                        units=num_hidden_units,
                        activation=tf.nn.relu,
                        name=str(mlp_top_hidden_layer_scope.name))
                    mlp_logits = dense_layer(mlp_input)
        ...
        with tf.compat.v1.variable_scope('logits', reuse=tf.compat.v1.AUTO_REUSE) as logits_scope:
            dense_layer = tf.keras.layers.Dense(
                units=1,
                activation=None,
                name=str(logits_scope.name))
            self._logits = dense_layer(mlp_logits)
    ...
    def _create_optimizer(self):
        ...
        if self.tf or self._optimizer_type == 'adam':
            ...
            optimizer = tf.compat.v1.train.AdamAsyncOptimizer(
                learning_rate=self._learning_rate,
                beta1=0.9,
                beta2=0.999,
                epsilon=1e-8)
        elif self._optimizer_type == 'adagraddecay':
            optimizer = tf.compat.v1.train.AdagradDecayOptimizer(
                learning_rate=self._learning_rate,
    ...
    def _create_metrics(self):
        self.acc, self.acc_op = tf.compat.v1.metrics.accuracy(labels=self._label,
                                                    predictions=self.output)
        self.auc, self.auc_op = tf.compat.v1.metrics.auc(labels=self._label,
...
def train(sess_config,
    ...
    scaffold = tf.compat.v1.train.Scaffold(
        local_init_op=tf.group(tf.compat.v1.local_variables_initializer(), data_init_op),
        saver=tf.compat.v1.train.Saver(max_to_keep=args.keep_checkpoint_max, sharded=True))
...
def main(tf_config=None, server=None):
    ...
    dataset_output_types = train_dataset.element_spec
    dataset_output_shapes = test_dataset.element_spec
    ...
    if args.smartstaged and not args.tf:
        ...
        next_element = tf.compat.v1.staged(next_element, num_threads=4, capacity=40)
        sess_config.graph_options.optimizer_options.do_smart_stage = True
        hooks.append(tf.compat.v1.make_prefetch_hook())
    ...
    model = DLRM(dense_column=dense_column,
                 ...
                 use_recstore=args.use_recstore)
...
def get_arg_parser():
    ...
    parser.add_argument("--group_embedding", \
                        ...
                        default=None)
    parser.add_argument("--use_recstore", \
                        help='Whether to enable RecStore backend for embedding storage. Default to False.',
                        type=boolean_string,
                        default=False)
...
def generate_cluster_info(TF_CONFIG):
    ...
    server = tf.compat.v1.distribute.Server(cluster,
                                  job_name=task_type,
