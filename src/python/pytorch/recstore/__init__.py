...
# Load the extension on import.  Failure here is fatal (any subsequent
# `torch.ops.recstore_ops.<op>` would raise AttributeError), so we let the
# OSError propagate.  The pure-Python submodules (DistTensor, DistEmb, ...)
# do not need the extension to import, but their *use* does.
_loaded_so_path = _ensure_native_loaded()
if _loaded_so_path:
    # Only log when we actually loaded something; silent on subsequent imports.
